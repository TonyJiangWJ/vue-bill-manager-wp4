<template>
  <div>
    ...
    <Row type="flex" justify="center" align="middle">
      <i-col offset="8">
        <Button type="success" @click="calculate">计算</Button>&nbsp;
        <Checkbox v-model="checkBox.showOldTax">显示旧个税信息</Checkbox>
      </i-col>
    </Row>
    <Divider />
    <Row type="flex" justify="center" align="middle">
      <i-col :xs="12" :sm="8">
        <Checkbox v-model="checkBox.customSocialPoint">自定义</Checkbox>
        <label>社&nbsp;&nbsp;&nbsp;保起征点:</label>
      </i-col>
      <i-col :xs="12" :sm="8">
    ...
    <Row type="flex" justify="center" align="middle">
      <i-col :xs="12" :sm="8">
        <Checkbox v-model="checkBox.customProvidentPoint">自定义</Checkbox>
        <label>公积金起征点:</label>
      </i-col>
      <i-col :xs="12" :sm="8">
        <Row type="flex" justify="center" align="middle">
          <i-col :sm="4">
            <InputNumber
              v-model="customProvidentPrecent"
              :max="100"
    ...
    <Row type="flex" justify="center" align="middle" v-if="checkBox.showOldTax">
      <i-col :xs="12" :sm="8">
        <Checkbox v-model="checkBox.customTaxPoint">自定义</Checkbox>
        <label>旧个税起征点:</label>
      </i-col>
      <i-col :xs="12" :sm="8">
    ...
    <Row type="flex" justify="center" align="middle">
      <i-col :xs="12" :sm="8">
        <Checkbox v-model="checkBox.customNewTaxPoint">自定义</Checkbox>
        <label>新个税起征点:</label>
      </i-col>
      <i-col :xs="12" :sm="8">
  ...
  methods: {
    calculate: function() {
      ...
      this.afterInsurance = this.forTax = (
        this.beforeTax -
        this.socialInsurancePoint * (this.config.unemployScale + this.config.endowmentScale + this.config.medicalScale) -
    ...
    getTaxFee: function(taxAmount, step, preStep, type) {
      ...
      if (type === 'old') {
        ...
        this.oldTaxLadderTips =
          '当前税率:' + step.percent * 100 + '% ' + '税阶税费：' + ((taxAmount - preStep.amount) * step.percent).toFixed(2)
        this.$debug(this.oldTaxLadderTips)
      } else {
        this.newTaxCurrentLadder = step.percent
        this.newTaxLadderTips =
          '当前税率:' + step.percent * 100 + '% ' + '税阶税费：' + ((taxAmount - preStep.amount) * step.percent).toFixed(2)
